<x-app-layout>
    ...
    <div class="grid  lg:grid-cols-2 sm:grid-cols-2  justify-center ">
        ...
        <div class="p-4 max-w-full">
            <div class="flex rounded-lg h-full dark:bg-gray-800 bg-[#EB4B5A]  p-8 flex-col">
                ...
                <form class="w-full mx-auto" action="{{ route('scan.out.store') }}" method="POST">
                @csrf
                @method('POST')    
                    ...
                    <select id="item_name" name="item_id" class="bg-gray-50 border mb-2 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        <option class="py-2" value="{{ $item->qr_code }}" data-desc="{{ $item->description }}" data-stock="{{ $item->stock }}">{{ $item->name }} - {{ $item->qr_code }}</option>
                    ...
                    <div class="mb-2">
                        <label  class="block  mb-2 text-sm font-medium text-white dark:text-white">Stock Saat ini</label>
                        <input type="number" min="1" value="" disabled id="in-stock" class="bg-gray-50 disabled border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    </div>
                    <div class="mb-2">
    ...
    <script>
        document.getElementById('item_name').addEventListener('change', function() {
            ...
            var in_stock = selectedOption.getAttribute('data-stock');
            // var in_stock = selectedOption.getAttribute('stock');
        ...
        document.getElementById('quantity').addEventListener('input', function() {
            var inStock = parseInt(document.getElementById('in-stock').value);
